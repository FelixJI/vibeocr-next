...
| 产品发布 | `.ci/project.json` 精确要求 full/delta NUPKG、Portable、feed、component lock、component identities 与 SBOM 七项资产，并拒绝 Setup/sidecar | release build 使用上一正式 full 生成单跳 delta，并以真实 Portable check/download/apply/restart 验证 delta 请求与 state 保留；解包/Web smoke 仍不得替代该门禁 |
