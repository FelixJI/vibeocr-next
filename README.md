...
- 在“设置 → 运行环境”中选择“推理设备”（CPU 或 CUDA GPU），并按需勾选“文档解析
  （PaddleOCR/MinerU）”“CUDA GPU 运行时”等组件。这些选择只是安装意图：点击“预览安装
...
- PaddleOCR 与 MinerU 可在支持范围内独立选择，实际必需依赖与新增/保留/移除以安装计划为准。
  它们与基础环境使用相互隔离的解释器，兼容的依赖工件会复用已验证的下载缓存；缓存复用
  不等于安装目录共享，也不承诺所有场景的磁盘占用都会减少。
