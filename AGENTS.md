...
- 版本唯一事实源是 `repository.json`，`scripts/sync_version.py` 派生 App csproj。桥接期正式资产精确为 Next win64 legacy ZIP/sha256、Velopack full nupkg、Setup/sha256、Portable ZIP、`releases.win.json`、component lock、component identities 与 SPDX SBOM，项目 smoke 必须拒绝额外资产。legacy ZIP、Python `updater.exe`、对应 layout/cleaner 与旧替换链至少保留两个正式版本或 90 天（以较长者为准），窗口结束后再由独立清理 PR 删除。
