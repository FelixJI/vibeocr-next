...
- 版本唯一事实源是 `repository.json`，`scripts/sync_version.py` 派生 App csproj。正式资产精确为 Velopack full nupkg、Setup/sha256、Portable ZIP、`releases.win.json`、component lock、component identities 与 SPDX SBOM，项目 smoke 必须拒绝额外资产。应用内更新只使用 Velopack；Portable 仅提示手动下载，不自动获取或启动 Setup。
