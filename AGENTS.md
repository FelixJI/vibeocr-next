...
- quality=`python scripts/check_quality.py`；E2E 同时运行 Platform tests 与 `scripts/test_app_ci.ps1` 的 fail-closed App tests；随后 `scripts/build-release.ps1` 和 `scripts/release_smoke.py` 构建并验证真实候选。
