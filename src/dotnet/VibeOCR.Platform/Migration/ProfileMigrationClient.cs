...
/// <c>app_settings.json</c>, writes a schema-labelled backup, and replaces atomically.
...
public static class ProfileMigrationClient
{
  ...
  public static MigrationResult MigrateConfig(PortableLayout layout)
  {
    ...
    if (data.TryGetValue("schema_version", out object? existing))
    {
      ...
      sourceSchemaVersion = Math.Max(version, 0);
    ...
    try
    {
      backupPath = WriteSchemaBackup(layout, path, sourceSchemaVersion);
    }
  ...
  private static string WriteSchemaBackup(
      ...
      int sourceSchemaVersion)
  {
    ...
    string backup = Path.Combine(
        path.DirectoryName!,
        $"{Path.GetFileNameWithoutExtension(path.Name)}.pre-migrate-schema-v{sourceSchemaVersion}"
            + $"-to-v{CurrentSchemaVersion}{path.Extension}.bak");
